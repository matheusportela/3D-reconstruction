...
rawP = load( fullfile( dataDir, 'morp.mat') );
...
for ii=idx(:)'
    ...
    % the faster to load
    filename = fullfile( dataDir, sprintf( '%d.jpg', ii-1 ) );
    if exist( filename, 'file' )~=2
        % Try PPM
        filename = fullfile( dataDir, sprintf( '%d.ppm', ii-1 ) );
        if exist( filename, 'file' )~=2
            % Failed
            error( 'SpaceCarving:ImageNotFound', ...
                'Could not find image %d (''%d.jpg/.ppm'')', ...
                ii, ii );
        end
    end
